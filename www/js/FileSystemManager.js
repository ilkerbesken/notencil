...
class FileSystemManager {
    constructor() {
        ...
        if (this.isCapacitor) {
            this.mode = 'native';
            console.log('[FileSystemManager] Capacitor tespit edildi. Native mod aktif.');
        }
    }
    ...
    async init() {
        ...
        if (this.mode === 'native') {
            if (this.isCapacitor) {
                await this._syncFoldersToCapacitor();
    ...
    // Capacitor desteği
    async _getCapacitorPath(pathSegments) {
        return pathSegments.join('/');
    }

    async _ensureCapacitorDirectory(pathSegments) {
        if (!this.isCapacitor || pathSegments.length === 0) return;
        ...
        for (const segment of pathSegments) {
            ...
            try {
                await Filesystem.mkdir({
                    path: currentPath,
                    directory: window.Capacitor.Plugins.Directory.Documents,
                    recursive: false
                });
            } catch (e) {
                // Klasör zaten varsa hata verir, yoksayalım
            }
        }
    }
    ...
    async _saveBoardToCapacitor(key, value) {
        ...
        await this._ensureCapacitorDirectory(folders);
    ...
    async _saveMetaToCapacitor(key, value) {
        const safeKey = key.replace(/[^a-zA-Z0-9_\-]/g, '_');
        const path = `_meta/${safeKey}.json`;
        await this._ensureCapacitorDirectory(['_meta']);
        ...
        await Filesystem.writeFile({
            ...
            directory: window.Capacitor.Plugins.Directory.Documents,
            encoding: window.Capacitor.Plugins.Encoding.UTF8
        });
    }

    async _syncFoldersToCapacitor() {
        ...
        for (const folder of sortedFolders) {
            ...
            await this._ensureCapacitorDirectory(pathSegments);
        }
    }

    async _loadBoardFromCapacitor(boardId) {
        const pathSegments = this._getBoardFilePath(boardId);
        ...
        try {
            ...
            if (contentStart.startsWith(APP_CONFIG.SIGNATURE || 'notencil!')) {
                const headerLength = (APP_CONFIG.SIGNATURE || 'notencil!').length;
                const compressed = uint8.slice(headerLength);
                jsonStr = pako.ungzip(compressed, { to: 'string' });
            } else {
    ...
    async _loadMetaFromCapacitor(key) {
        const safeKey = key.replace(/[^a-zA-Z0-9_\-]/g, '_');
        const path = `_meta/${safeKey}.json`;
        const { Filesystem } = window.Capacitor.Plugins;
    ...
    async _removeItemFromCapacitor(key) {
        const { Filesystem } = window.Capacitor.Plugins;
        try {
    ...
    async saveItem(key, value, skipNative = false, fromSync = false) {
        ...
        if (!skipNative && (this.isCapacitor || (this.mode === 'native' && this.dirHandle))) {
    ...
    async _saveToNative(key, value) {
        if (!this.isCapacitor && !this.dirHandle) return;

        try {
            if (key.startsWith('wb_content_')) {
                if (this.isCapacitor) {
                    await this._saveBoardToCapacitor(key, value);
                } else {
                ...
                if (this.isCapacitor) {
                    await this._saveMetaToCapacitor(key, value);
                    await this._syncFoldersToCapacitor();
                } else {
    ...
    async getItem(key, defaultValue) {
        ...
        if (this.mode === 'native') {
            try {
                if (key.startsWith('wb_content_')) {
                    const boardId = key.replace('wb_content_', '');
                    const val = this.isCapacitor 
                        ? await this._loadBoardFromCapacitor(boardId)
                        : await this._loadBoardFromNative(boardId);
                    if (val) {
                    ...
                    const val = this.isCapacitor
                        ? await this._loadMetaFromCapacitor(key)
                        : await this._loadMetaFromNative(key);
                    if (val) {
                         return val;
                    }
                }
    ...
    async removeItem(key) {
        ...
        if (this.mode === 'native') {
            if (this.isCapacitor) {
                await this._removeItemFromCapacitor(key);
            } else if (this.dirHandle) {
                if (key.startsWith('wb_content_')) {
