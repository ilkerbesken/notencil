    ...
    _setupMobileStorageButtons() {
        ...
        if (btnExport) {
            btnExport.onclick = async () => {
                ...
                // Use FileSystemManager for export if available
                if (window.fileSystemManager && window.fileSystemManager.exportAll) {
                    await window.fileSystemManager.exportAll();
                    Utils.showToast('Veriler dışa aktarıldı.', 'info');
                } else {
                    Utils.showToast('Dışa aktarma şu an kullanılamıyor.', 'warning');
                }
        ...
        if (btnImportJson) {
            btnImportJson.onclick = async () => {
                if (window.fileSystemManager && window.fileSystemManager.importAll) {
                    const success = await window.fileSystemManager.importAll();
                    if (success) {
                        Utils.showToast('Veriler başarıyla içe aktarıldı.', 'success');
                        await this.initAsync();
                    }
                } else {
                    Utils.showToast('İçe aktarma şu an kullanılamıyor.', 'warning');
                }
            };
        }
    }
